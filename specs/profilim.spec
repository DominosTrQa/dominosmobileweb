Profilim
=====================

     
Profilim - Ye-Kazan - Sayfa Kontrolü
-----------------------------------------------------
Tags:profilim_YeKazanSayfaKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Ye-Kazan butonuna basılır
* Ye-Kazan sayfasındaki elementlerin geldiği kontrol edilir
* Anasayfadaki tabın geldiği kontrol edilir
* Dominos logosuna tıklanır ve anasayfaya yönlendirildiği kontrol edilir


Profilim - Ye Kazan Üyelik Koşulları - Sayfa Kontrolü
-----------------------------------------------------
Tags:profilim_YeKazanUyelikKosullariSayfaKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Ye-Kazan butonuna basılır
* Ye-Kazan Üyelik Koşulları kontrol edilir


Profilim - Üyelik Bilgilerim - Zorunlu Alanların Kontrolü
-----------------------------------------------------
Tags:profilim_UyelikBilgilerimZorunluAlanlarinKontrolu

* Giriş Yap butonuna tıklanır
* Kullanıcı e-posta ve şifresi ile giriş yapılır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Profilim butonuna basılır
* Üyelik Bilgilerimde dolu gelmesi gereken alanlar kontrol edilir


Profilim - Üyelik Bilgilerim - Değiştirilebilir Alanların Kontrolü
-----------------------------------------------------
Tags:profilim_UyelikBilgilerimDegistirilebilirAlanlarinKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Profilim butonuna basılır
* Üyelik Bilgilerimde değiştirilebilir alanların geldiği kontrol edilir


Profilim - Üyelik Bilgilerim - Üyelikten Ayrılmak İçin Tıklayon Kontrolü
-----------------------------------------------------
Tags:profilim_UyelikBilgilerimUyeliktenAyrilmakIcinTiklayinKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Profilim butonuna basılır
* Üyelikten Ayrılmak İçin Tıklayın butonuna tıklanır ve gelen popup kontrol edilir


Profilim - Üyelik Bilgilerim - Üyelikten Ayrılmanın Başarılı Bir Şekilde Yapılması Kontrolü
-------------------------------------------------
Tags:profilim_UyelikBilgilerimUyeliktenAyrilmaninBasariliBirSekildeYapilmasiKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Profilim butonuna basılır
* Üyelikten Ayrılmak İçin Tıklayın butonuna tıklanır ve başarılı bir şekilde işlem tamamlanır


Profilim - Üyelik Bilgilerim - Üyelikten Ayrılmaktan Vazgeçme Kontrolü
-------------------------------------------------
Tags:profilim_UyelikBilgilerimUyeliktenAyrilmaktanVazgecmeKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Profilim butonuna basılır
* Üyelikten Ayrılmak İçin Tıklayın butonuna tıklanır ve Vazgeç butonunun çalıştığı kontrol edilir


Profilim - Üyelik Bilgilerim - KVKK Aydınlatma Metni Kontrolü
-------------------------------------------------
Tags:profilim_UyelikBilgilerimKVKKAydinlatmaMetniKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Profilim butonuna basılır
* Üyelik Bilgilerimde ki KVKK Aydınlatma Metni kontrol edilir


Profilim - Üyelik Bilgilerim - Açık Rıza Metni Kontrolü
-------------------------------------------------
Tags:profilim_UyelikBilgilerimaAcikRizaMetniKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Profilim butonuna basılır
* Üyelik Bilgilerimde ki Açık Rıza Metni kontrol edilir


Profilim - Üyelik Bilgilerim - Ye-Kazan Sadakat Programı Üyelik Koşulları Metni Kontrolü
-------------------------------------------------
Tags:profilim_UyelikBilgilerimayeKazanSadakatProgramiUyelikKosullariMetniKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Profilim butonuna basılır
* Ye-Kazan Üyelik Koşulları kontrol edilir


Profilim - Siparişlerim - Sipariş Alanları Kontrolü
-------------------------------------------------
Tags:profilim_UyelikBilgilerimSiparislerimSiparisAlanlariKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Siparişlerim butonuna basılır
* Siparişlerim sayfasındaki alanların geldiği kontrol edilir


Profilim - Pizza Takip - Zorunlu Alan Kontrolü
-------------------------------------------------
Tags:profilim_PizzaTakipZorunluAlanKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Pizza Takip butonuna basılır
* Pizza Takip sayfasında Görüntüle butonuna basılır ve uyarı alındığı görülür


Profilim - Pizza Takip - Dijital Pizza Takip Sistemi Yazısının Kontrolü
-------------------------------------------------
Tags:profilim_PizzaTakipDijitalPizzaTakipSistemiYazisininKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Pizza Takip butonuna basılır
* Pizza Takip sayfasında ki Dijital Pizza Takip Sistemi yazısının geldiği kontrol edilir


Profilim - Notlarım - Yeni Not Ekle Popup Kontrolu
-------------------------------------------------
Tags:profilim_NotlarimYeniNotEklePopupKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Notlarım butonuna basılır
* Yeni Not Ekle butonuna basılır ve popup açıldığı kontrol edilir


Profilim - Notlarım - Yeni Not Ekleme Kontrolu
-------------------------------------------------
Tags:profilim_NotlarimYeniNotEklemeKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Notlarım butonuna basılır
* Yeni Not Ekle butonuna basılır ve popup açıldığı kontrol edilir
* Yeni not eklenir ve eklendiği kontrol edilir


Profilim - Notlarım - Eklenen Notu Düzenleme Popup Kontrolü
-------------------------------------------------
Tags:profilim_NotlarimEklenenNotuDuzenlemePopupKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Notlarım butonuna basılır
* Yeni Not Ekle butonuna basılır ve popup açıldığı kontrol edilir
* Yeni not eklenir ve eklendiği kontrol edilir
* Not Düzenle butonuna basılır ve popup açıldığı kontrol edilir


Profilim - Notlarım - Var Olan Notu Değiştirme Kontrolü
-------------------------------------------------
Tags:profilim_NotlarimVarOlanNotuDegistirmeKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Notlarım butonuna basılır
* Yeni Not Ekle butonuna basılır ve popup açıldığı kontrol edilir
* Yeni not eklenir ve eklendiği kontrol edilir
* Eklenen not düzenlenir ve değişikliklerin yansıdığı kontrol edilir


Profilim - Notlarım - Var Olan Notu Değiştirmekten Vazgeçme Kontrolü
-------------------------------------------------
Tags:profilim_NotlarimVarOlanNotuDegistirmektenVazgecmeKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Notlarım butonuna basılır
* Yeni Not Ekle butonuna basılır ve popup açıldığı kontrol edilir
* Yeni not eklenir ve eklendiği kontrol edilir
* Not Düzenle butonuna basılır ve popup açıldığı kontrol edilir
* Not değiştirmekten Vazgeç butonuna basılır ve değişiklik olmadığı kontrol edilir


Profilim - Notlarım - Var Olan Notu Silme Kontrolü
-------------------------------------------------
Tags:profilim_NotlarimVarOlanNotuSilmeKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Notlarım butonuna basılır
* Yeni Not Ekle butonuna basılır ve popup açıldığı kontrol edilir
* Yeni not eklenir ve eklendiği kontrol edilir
* Not Düzenle butonuna basılır ve popup açıldığı kontrol edilir
* Not silinir ve silindiği kontrol edilir


Profilim  - Çıkış Yap - Var Olan Kullanıcı ile Çıkış Kontrolü
----------------------------------------------------------------------
Tags:profilim_CikisYapVarOlanKullaniciileCikisKontrolu

* Giriş Yap butonuna tıklanır
* Kullanıcı e-posta ve şifresi ile giriş yapılır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Çıkış yapılır ve çıkış yapıldığı kontrol edilir


Profilim  - Çıkış Yap - Yeni Kullanıcı ile Çıkış Kontrolü
----------------------------------------------------------------------
Tags:profilim_CikisYapYeniKullaniciileCikisKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Çıkış yapılır ve çıkış yapıldığı kontrol edilir


Profilim  - Adreslerim - Sayfanın Açılması Kontrolü
----------------------------------------------------------------------
Tags:profilim_AdreslerimSayfaninAcilmasiKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Adreslerim butonuna basılır ve sayfanın geldiği kontrol edilir


Profilim  - Adreslerim - Manuel Yeni Adres Ekleme Kontrolü
----------------------------------------------------------------------
Tags:profilim_AdreslerimManuelYeniAdresEklemeKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Adreslerim butonuna basılır ve sayfanın geldiği kontrol edilir
* Yeni Adres Ekle butonuna tıklanır
* Adalar - Burgazada için adres bilgileri tamamlanır
* Yeni adresin eklendiği kontrol edilir


Profilim  - Adreslerim - İl İlçe Mahalle Değiştirme Kontrolü
----------------------------------------------------------------------
Tags:profilim_AdreslerimIlIlceMahalleDegistirmeKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Adreslerim butonuna basılır ve sayfanın geldiği kontrol edilir
* İl, ilçe, mahalle Düzenle butonuna tıklanır
* İl, İlçe, Mahalle (İstanbul, Sarıyer, Ayazağa) seçilir
* İl, ilçe, mahalle güncellemesi kontrol edilir


Profilim  - Adreslerim - Kayıtlı Adreslerden Seçme Kontrolü
----------------------------------------------------------------------
Tags:profilim_AdreslerimKayitliAdreslerdenSecmeKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Adreslerim butonuna basılır ve sayfanın geldiği kontrol edilir
* Yeni Adres Ekle butonuna tıklanır
* Adalar - Burgazada için adres bilgileri tamamlanır
* Yeni adresin eklendiği kontrol edilir
* Eklenen adres seçilir ve anasayfaya yönlendirdiği kontrol edilir


Profilim  - Adreslerim - Kayıtlı Adres Düzenle Kontrolü
----------------------------------------------------------------------
Tags:profilim_AdreslerimKayitliAdresDuzenleKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Adreslerim butonuna basılır ve sayfanın geldiği kontrol edilir
* Yeni Adres Ekle butonuna tıklanır
* Adalar - Burgazada için adres bilgileri tamamlanır
* Yeni adresin eklendiği kontrol edilir
* Eklenen adres düzenlenir ve değişikliğin yansıdığı kontrol edilir


Profilim  - Adreslerim - Kayıtlı Adres Sil Kontrolü
----------------------------------------------------------------------
Tags:profilim_AdreslerimKayitliAdresSilKontrolu

* Üye Ol butonuna tıklanır
* Random mail ve telefon ile üye olunur
* Mesafeli satış sözleşmesi onaylanır ve üye ol butonuna tıklanır
* Adrese Teslim servis tipi seçilir
* İl, İlçe, Mahalle (İstanbul, Adalar, Burgazada) seçilir
* Çerez Politikası kapatılır
* Hamburger menüye tıklanır
* Adreslerim butonuna basılır ve sayfanın geldiği kontrol edilir
* Yeni Adres Ekle butonuna tıklanır
* Adalar - Burgazada için adres bilgileri tamamlanır
* Yeni adresin eklendiği kontrol edilir
* Eklenen adres silinir ve silindiği kontrol edilir